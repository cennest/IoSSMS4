...
unlong *SMS4Encrypt(unlong *psrc, unlong lgsrc, unlong rk[])
{
    ...
    {
        pRet = psrc;
        printf("before En:---> 0x%08x, 0x%08x, 0x%08x, 0x%08x. \n",
               pRet[0], pRet[1], pRet[2], pRet[3]);
    ...
    printf("En:---> 0x%08x, 0x%08x, 0x%08x, 0x%08x. \n",
           pRet[0], pRet[1], pRet[2], pRet[3]);
...
unlong *SMS4Decrypt(unlong *psrc, unlong lgsrc, unlong derk[])
{
    ...
    printf("DE:---> 0x%08x, 0x%08x, 0x%08x, 0x%08x. \n",
           pRet[0], pRet[1], pRet[2], pRet[3]);
...
{
    SMS4Encrypt(psrc, length, ENRK);
    return psrc;
}

unlong *SMS4DecryptMain(unlong *psrc, size_t length)
{
    SMS4Decrypt(psrc, length, DERK);
    return psrc;
}
